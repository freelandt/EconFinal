...
poverty <- read_excel("~/R/Econometrics/Tables/Poverty2.xlsx")
aid <- read_excel("~/R/Econometrics/Tables/AidReceived.xlsx")
gdpgrow <- read_excel("~/R/Econometrics/Tables/GDPGrow.xlsx")
...
```{r}
data3 <- data2 %>% filter(aid >= 1)
model1 <- lm(growth~aid+year, data = data3)
summary(model1)
plot(model1, which = 1)
